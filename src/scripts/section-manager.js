...
class ShopifySectionManager {
  ...
  init() {
    ...
    this.container =
      document.getElementById("dynamic-sections") || this.createContainer();
  }
  ...
  loadSection = async (sectionId, context = window.location.pathname) => {
    try {
      ...
      if (!response.ok) {
        throw new Error(
          `Failed to load section ${sectionId}: ${response.status}`,
        );
      }
  ...
  showLoadingState = (sectionId) => {
    ...
    loadingDiv.className =
      "section-loading flex justify-center items-center py-16";
